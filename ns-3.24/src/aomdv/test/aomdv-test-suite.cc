 ...
 */
#include "ns3/test.h"
#include "ns3/aomdv-neighbor.h"
#include "ns3/aomdv-packet.h"
#include "ns3/aomdv-rqueue.h"
#include "ns3/aomdv-rtable.h"
...
{
  TypeHeaderTest () : TestCase ("AOMDV TypeHeader") 
  {
  }
  virtual void DoRun ()
  {
    TypeHeader h (AOMDVTYPE_RREQ);
    NS_TEST_EXPECT_MSG_EQ (h.IsValid (), true, "Default header is valid");
    NS_TEST_EXPECT_MSG_EQ (h.Get (), AOMDVTYPE_RREQ, "Default header is RREQ");

    Ptr<Packet> p = Create<Packet> ();
    p->AddHeader (h);
    TypeHeader h2 (AOMDVTYPE_RREP);
...
{
  RreqHeaderTest () : TestCase ("AOMDV RREQ") 
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepHeaderTest () : TestCase ("AOMDV RREP") {}
  virtual void DoRun ()
  {
...
{
  RrepAckHeaderTest () : TestCase ("AOMDV RREP-ACK")
  {
  }
  virtual void DoRun ()
  {
...
{
  RerrHeaderTest () : TestCase ("AOMDV RERR")
  {
  }
  virtual void DoRun ()
  {
...
//-----------------------------------------------------------------------------
/// Unit test for AOMDV routing table entry
...
struct AomdvRqueueTest : public TestCase
{
  AomdvRqueueTest () : TestCase ("Rqueue"), q (64, Seconds (30)) {}
  virtual void DoRun ();
  void Unicast (Ptr<Ipv4Route> route, Ptr<const Packet> packet, const Ipv4Header & header) {}
  void Error (Ptr<const Packet>, const Ipv4Header &, Socket::SocketErrno) {}
  void CheckSizeLimit ();
...
AomdvRqueueTest::DoRun ()
{
  ...
  h.SetDestination (Ipv4Address ("1.2.3.4"));
  h.SetSource (Ipv4Address ("4.3.2.1"));
  Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback (&AomdvRqueueTest::Unicast, this);
  Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback (&AomdvRqueueTest::Error, this);
  ...
  Simulator::Schedule (q.GetQueueTimeout () + Seconds (1), &AomdvRqueueTest::CheckTimeout, this);
...
void
AomdvRqueueTest::CheckSizeLimit ()
{
...
void
AomdvRqueueTest::CheckTimeout ()
{
  NS_TEST_EXPECT_MSG_EQ (q.GetSize (), 0, "Must be empty now");
}
//-----------------------------------------------------------------------------
/// Unit test for AOMDV routing table entry
struct AomdvRtableEntryTest : public TestCase
{
  AomdvRtableEntryTest () : TestCase ("RtableEntry") {}
  virtual void DoRun ()
  {
    Ptr<NetDevice> dev;
...
//-----------------------------------------------------------------------------
/// Unit test for AOMDV routing table
struct AomdvRtableTest : public TestCase
{
  AomdvRtableTest () : TestCase ("Rtable") {}
  virtual void DoRun ()
  {
...
//-----------------------------------------------------------------------------
class AomdvTestSuite : public TestSuite
{
public:
  AomdvTestSuite () : TestSuite ("routing-aomdv", UNIT)
  {
    ...
    AddTestCase (new AomdvRqueueTest, TestCase::QUICK);
    AddTestCase (new AomdvRtableEntryTest, TestCase::QUICK);
    AddTestCase (new AomdvRtableTest, TestCase::QUICK);
  }
} g_aomdvTestSuite;
