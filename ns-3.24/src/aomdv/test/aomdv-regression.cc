...
// Test suite
//-----------------------------------------------------------------------------
class AomdvRegressionTestSuite : public TestSuite
{
public:
  AomdvRegressionTestSuite () : TestSuite ("routing-aomdv-regression", SYSTEM) 
  {
    SetDataDir (NS_TEST_SOURCEDIR);
    // General RREQ-RREP-RRER test case
    AddTestCase (new ChainRegressionTest ("aomdv-chain-regression-test"), TestCase::QUICK);
...
ChainRegressionTest::ChainRegressionTest (const char * const prefix, Time t, uint32_t size, Time arpAliveTimeout) : 
  TestCase ("AOMDV chain regression test"),
  m_nodes (0),
  m_prefix (prefix),
  m_time (t),
  m_size (size),
...
ChainRegressionTest::CreateDevices ()
{
  ...
  // 2. Setup TCP/IP & AOMDV
  AomdvHelper aomdv; // Use default parameters here
  InternetStackHelper internetStack;
  internetStack.SetRoutingHelper (aomdv);
  ...
  streamsUsed += aomdv.AssignStreams (*m_nodes, streamsUsed);
  // AOMDV uses m_size more streams
