...
Ad Hoc On-Demand Distance Vector (AOMDV)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (AOMDV) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 AOMDV model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the AOMDV-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the AOMDV model lives in the directory `src/aomdv`.

Design
++++++

Class ``ns3::aomdv::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::aomdv::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::aomdv::RouteInput``,
...
this functionality are attributes of ``ns3::aomdv::RoutingProtocol``. 
...
AOMDV discovers routes on demand.  Therefore, the AOMDV model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in aomdv-rqueue.cc. A smart pointer to 
...
* This AOMDV implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from AOMDV-UU implementation and can be disabled.
...
the assertion from the AOMDV RFC that AOMDV works over UDP.  This model uses
